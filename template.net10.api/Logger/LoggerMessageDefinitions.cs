...
internal static class LoggerMessageDefinitions
{
    ...
    internal const string HandledPostProcess =
        "Handled Post Process {postProcessType} for the Request {requestType} in {time}.";

    /// <summary>
    ///     Log message template for a MediatR post-process that resulted in an error.
    /// </summary>
    internal const string HandledPostProcessError =
        "Handled Post Process {postProcessType} for the Request {requestType}, with state ERROR.";

    /// <summary>
    ///     Log message template for the start of a background task execution.
    /// </summary>
    internal const string BackgroundTaskStarted =
        "Started Background Task {backgroundTaskType} for the Request {requestType}.";

    /// <summary>
    ///     Log message template for a completed background task including elapsed time.
    /// </summary>
    internal const string BackgroundTaskCompleted =
        "Completed Background Task {backgroundTaskType} for the Request {requestType} in {time}.";

    /// <summary>
    ///     Log message template for background task exceptions.
    /// </summary>
    internal const string BackgroundTaskException =
