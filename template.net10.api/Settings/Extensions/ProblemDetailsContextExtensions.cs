...
internal static class ProblemDetailsContextExtensions
{
    extension(ProblemDetailsContext ctx)
    {
        ...
        {
            if (ProblemDetailsContext.ShouldHiddenDetails(env,
        ...
        /// </summary>
        /// <exception cref="ArgumentNullException"><see cref="ProblemDetails.Extensions" /> is <see langword="null" />.</exception>
        internal void AddMethodField()
        {
            if (ProblemDetailsContext.ContainsMethod(ctx.ProblemDetails.Extensions)) return;
        ...
        {
            if (ctx.NotContainsInstance())
                ctx.ProblemDetails.Instance = ctx.GetInstance();
        }
        ...
        {
            if (ProblemDetailsContext.ContainsRequestId(ctx.ProblemDetails.Extensions)) return;
        ...
        {
            if (ProblemDetailsContext.ContainsTraceId(ctx.ProblemDetails.Extensions)) return;
        ...
        {
            if (ProblemDetailsContext.ContainsCode(ctx.ProblemDetails.Extensions)) return;
        ...
        internal void UseHttpContextProblemDetails(ProblemDetails clientProblemDetails)
        {
            ...
            ctx.ProblemDetails.Extensions = clientProblemDetails.Extensions.Count > 0
                ? ProblemDetailsContext.MergeExtensions(ctx.ProblemDetails.Extensions, clientProblemDetails.Extensions)
        ...
        private bool NotContainsInstance()
        {
        ...
        {
            return $"{ctx.HttpContext.Request.Scheme}://{ctx.HttpContext.Request.Host}{ctx.HttpContext.Request.Path}";
        }
    }
}