...
internal static class AuthorizationBuilderExtensions
{
    extension(AuthorizationBuilder authorizationBuilder)
    {
        ...
        ///     <see langword="true" /> to apply strict production policies; <see langword="false" /> for development-friendly
        ///     policies.
        /// </param>
        internal void AddPolicies(bool isProduction)
        {
            ...
            foreach (var (policyName, requirements) in AuthorizationBuilder.GetPolicies(isProduction))
        ...
        private static Dictionary<string, List<ClaimRequirement>> GetPolicies(bool isProduction)
        {
            return new Dictionary<string, List<ClaimRequirement>>
            {
                ...
                [PoliciesConstants.UserResetPasswordPolicy] =
                    Create(ClaimIdentityConstants.UserResetPasswordClaimValue),
            ...
            {
                return isProduction
                    ? AuthorizationBuilder.CreateProductionPolicy(claimValue)
                    : AuthorizationBuilder.CreateDevelopmentPolicy(claimValue);
            }
        }
